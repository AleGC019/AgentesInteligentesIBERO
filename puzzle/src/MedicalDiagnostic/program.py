...
# Agregar nodos de síntomas y conectar con enfermedades, asignando pesos
grafo_diagnostico.add_edge("fiebre", "gripe", weight=0.9)
grafo_diagnostico.add_edge("tos", "gripe", weight=0.8)
grafo_diagnostico.add_edge("dolor_de_cabeza", "gripe", weight=0.7)
grafo_diagnostico.add_edge("dolor_muscular", "gripe", weight=0.7)
grafo_diagnostico.add_edge("fatiga", "gripe", weight=0.6)

grafo_diagnostico.add_edge("tos", "resfriado_comun", weight=0.9)
...
grafo_diagnostico.add_edge("estornudos", "alergia_estacional", weight=0.9)
grafo_diagnostico.add_edge("picazon_en_los_ojos", "alergia_estacional", weight=0.8)
...
grafo_diagnostico.add_edge("ojos_llorosos", "alergia_estacional", weight=0.6)

grafo_diagnostico.add_edge("nauseas", "gastroenteritis", weight=0.9)
grafo_diagnostico.add_edge("diarrea", "gastroenteritis", weight=0.9)
grafo_diagnostico.add_edge("dolor_abdominal", "gastroenteritis", weight=0.8)
grafo_diagnostico.add_edge("vomitos", "gastroenteritis", weight=0.7)
...
grafo_diagnostico.add_edge("dolor_facial", "sinusitis", weight=0.9)
...
umbral_tolerancia = 0.5  # Por ejemplo, se pueden ajustar según sea necesario
...
def diagnosticar_con_grafo(grafo, sintomas):
    ...
    # Evaluar coincidencias completas
    for enfermedad in enfermedades_posibles.keys():
        sintomas_conocidos = list(grafo.neighbors(enfermedad))  # Convertir a lista
        if set(sintomas) == set(sintomas_conocidos):
...
def calcular_probabilidad(sintomas_ingresados, sintomas_conocidos):
    ...
    total_sintomas_conocidos = len(set(sintomas_conocidos))  # Total de síntomas conocidos
...
sintomas_ingresados = input("Ingrese los síntomas separados por comas: ").strip().split(", ")
...
if enfermedades_posibles:
    ...
    for enfermedad, probabilidad in resultados:
        ...
        print(f"{enfermedad}: Probabilidad: {probabilidad:.2f}%")

    # Mostrar coincidencias completas
    if coincidencias_completas:
        print("\nCoincidencias completas encontradas:")
...
# -----------------------------------------------------------------------------------------------------
# IMPRESION PORCENTUAL DE PASTEL
# # Diagnóstico basado en síntomas proporcionados con tolerancia
# def diagnosticar_con_grafo(grafo, sintomas):
#     enfermedades_posibles = {}
#     for sintoma in sintomas:
#         if sintoma in grafo:
#             for enfermedad in grafo.neighbors(sintoma):
#                 peso = grafo[sintoma][enfermedad]['weight']
#                 if peso >= umbral_tolerancia:  # Aplicar la tolerancia
#                     if enfermedad in enfermedades_posibles:
#                         enfermedades_posibles[enfermedad] += peso
#                     else:
#                         enfermedades_posibles[enfermedad] = peso
...
#
# # Diagnóstico
# enfermedades_posibles = diagnosticar_con_grafo(grafo_diagnostico, sintomas_ingresados)
#
# # Mostrar diagnósticos y porcentajes de probabilidad
# if enfermedades_posibles:
#     puntuacion_total = sum(enfermedades_posibles.values())
#     print("\nDiagnóstico(s) posible(s):")
#     for enfermedad, puntuacion in enfermedades_posibles.items():
#         probabilidad = (puntuacion / puntuacion_total) * 100
#         print(f"{enfermedad.capitalize()}: {puntuacion:.2f} (Probabilidad: {probabilidad:.2f}%)")
# else:
#     print("No se encontraron diagnósticos posibles con los síntomas proporcionados.")

# -----------------------------------------------------------------------------------------------------
# IMPRESION DE PESOS DEL GRAFO
# if diagnosticos:
#     mejores_candidatos = sorted(diagnosticos.items(), key=lambda x: x[1], reverse=True)
#     print("\nDiagnóstico(s) posible(s):")
#     for enfermedad, puntuacion in mejores_candidatos:
#         print(f"{enfermedad.capitalize()}: {puntuacion:.2f}")
# else:
#     print("No se encontraron diagnósticos posibles con los síntomas proporcionados.")